...
public class Properties extends HashMap<String,String>
{
	 ...
	 */
	  ...
	  * Properties object.
	  * @param prefix the prefix string to use as a filter on existing entry
	  * keys.
		 ...
		 try {
			 ...
			 if (!userProperties.exists())
				 throw new IOException("Missing properties file for "+
						 ...
						 userProperties.getAbsolutePath());
			 p.load(new FileInputStream(userProperties));
		 } catch (Exception e) {
			 System.err.println("Caught exception while loading properties: "
					 + e.getMessage());
			 throw new RuntimeException("Could not find properties at "
					 + conf_dir+"/"+ className + ".properties");
		 }
		 finally
		 {
